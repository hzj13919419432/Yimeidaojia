...
function _getCountryHall() {
  var cid = api.pageParam.cid;
  var bandHall = $api.getStorage('bandHall');
  $.each(bandHall,function (index,item) {
    if(item.C_Id == cid){
      $('#head-title').text(item.C_NameCN);
    }
  });
  var result = getCountryHall({cid:cid});
  result.then(function (res) {
    if(res.status ==200){
      $.each(res.info,function (index,item) {
        $.each(item.product,function (index2,item2) {
          res.info[index].product[index2].P_ImgUrl = imageUrl+item2.P_ImgUrl;
        });
      });
      var html = template('sbList_tmpl',{sbList:res.info});
      $('#sbList').html(html);
    }
  })
}
